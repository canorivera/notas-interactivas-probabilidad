...
st.markdown(r''' 
    ...
    - $P[a \leq x \leq b] = \intop_{a}^b f_x(x) dx$
    
    - $P[x = a] = \intop_{a}^a f_x(x) dx = 0$
...
st.markdown(r'''
    ...
    - $F_x(X) = P(X \leq x) = \int_{-\infin}^x f_x(t)dt$
...
- Si $X$ es una v.a.c., $E[g(X)] = \int_{x \in S_x} g(x) \cdot f_x(X) dx$
''')
